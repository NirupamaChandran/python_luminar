...
# movie having highest runtime
highest_runtime=max(movies,key=lambda d:int(d.get("runtime")))
print(highest_runtime.get("title"))
...
# maximum movie released year
value_key=[(v,k) for k,v in year_count.items()]
print(max(value_key))

# movie with greatest runtime
grt_runtime=[mv.get("title") for mv in movies if int(mv.get("runtime"))>150]
print(grt_runtime)


# biography movies
bio_movies=[mv.get("title") for mv in movies if "Biography" in mv.get("genres")]
print(bio_movies)

#sci-fi movies
sci_drama=[mv.get("title") for mv in movies if "Sci-Fi" in mv.get("genres") and "Drama" in mv.get("genres")]
print(sci_drama)
...
for mv in movies:
    ...
    for g in genres:
        if g in genres_count:
            ...
            genres_count[g]=1
# print(genres_count)
max_genres=[(v,k) for k,v in genres_count.items()]
print(max(max_genres))